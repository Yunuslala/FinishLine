...
const postData = async (json) => {
  try {
    ...
    const indexingPromises = json.map(async (item) => {
      const indexingResult = await index.saveObject(item, {
...
searchingRouter.post("/post/aloglia", async (req, res) => {
  try {
    ...
    const saveData = await SearchIndex.saveObjects(AllData, {
      autoGenerateObjectIDIfNotExist: true,
    });
    ...
    res
      .status(201)
      .send({ msg: "all data is stored in algolia-search", saveData });
  } catch (error) {
    console.log(error);
